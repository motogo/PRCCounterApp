...
#python MultiPhaseLineByFile5.py starttime=1 logfile=d:\temp temppath=d:\temp datafile=d:\prccounterapp\results\AutogeneratedResultFile_142298108.dat headerfile=d:\temp\header.dat debug=true colsplit=, rowsplit=; timetype=sec outlier=1E-4
...
for line in sys.argv:
    ...
    if(len(linearr) > 1):
        ...
        if(line.startswith('outlier')):
            outlier = float(linearr[1])
            print(dateandtime()+'->Arguments->Set Outlier:%f' % outlier)
...
def collectGraph2(pl,axs,phase,time,devregname,timetype,plotn,scale,titlename,xtitelname,ytitelname,legendname,colorname,markername,outlier):
	...
	for row in phase:
		...
		if(outlier == 0):
			py.append(val)
		elif((ro < outlier) and (ro > -outlier)):
			py.append(val)
...
if(len(headerfile) > 0):
    ...
    for filelineno, line in enumerate(open(headerfile, encoding="utf-8")):
        ...
        elif(line.startswith('#outlier:')):
            outlier = float(line[9:])
            print(dateandtime()+'->Headerfile->Set Outlier:%f') % outlier
...
for dfile in dfiles:
    ...
    for filelineno, line in enumerate(open(dfile, encoding="utf-8")):
        ...
        if(line.startswith('#starttime:') and (float(start_time) < -1)):
            start_time = line[10:]
            print(dateandtime()+'->Datafile->Set start mjd:'+start_time)
        elif(line.startswith('#endtime:') and (float(end_time) < -1)):
            end_time = line[8:]
            print(dateandtime()+'->Datafile->Set end mjd:'+end_time)
        elif(line.startswith('#offset:')):
            offset = float(line[8:])
        elif(line.startswith('#titel:')):
            titelname = line[7:]
            ...
            outlier = float(line[9:])
            print(dateandtime()+'->Datafile->Set Outlier:%f') % outlier
        elif(line.startswith('#measname:')):
            phase.clear()
            time.clear()
            measname = line[10:]
    ...
    if(enddone==False):
        ...
        collectGraph2(fig,axs,phase,time,measname,timetype,-1,scale,titelname,xtitelname,ytitelname,legend,color,marker,outlier)
        phase.clear()
